...
<?php
                                  ...
                                  echo '<div ><p><p><a href="files/'.$filename.'" download></a> .</p>';
?>


<div class="list-group"><a href="#" class="list-group-item active"><h4 class="list-group-item-heading">НАЗВАНИЕ ДОКУМЕНТА</h4>
     <p class="list-group-item-text">описание документа.</p></a><a href="#" class="list-group-item">
...
<?php
	...
	mysql_select_db($dbName) or die(mysql_error()); 
	$query = "SELECT * FROM $userstable WHERE `word` = 'объявлен' Limit 100";
	...
	while ($row=mysql_fetch_array($res)) 
	    ...
	    $site  = $row['namesite'];
            $title = $row['titlehref'];
            $URL   = 'http://'.$site;
            #echo ('<li><a href="pictlist.php?pic='.$site.'">'.$site.'</a></li>');
            echo '<h4 class="list-group-item-heading">'.$title.'</h4>';
            echo '<p class="list-group-item-text">'.$site.'</p></a><a href="'.$site.'" class="list-group-item">';
